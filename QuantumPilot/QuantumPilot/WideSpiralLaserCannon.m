...
+ (NSArray *)bulletsForLocation:(CGPoint)location direction:(NSInteger)direction {
    WideSpiralLaser *b1 = [[[WideSpiralLaser alloc] initWithLocation:ccp(location.x - 15, location.y) velocity:ccp(6, [self speed] * direction) centerX:location.x] autorelease];
    WideSpiralLaser *b2 = [[[WideSpiralLaser alloc] initWithLocation:ccp(location.x + 15, location.y) velocity:ccp(-6, [self speed] * direction) centerX:location.x] autorelease];
