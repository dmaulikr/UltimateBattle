...
- (NSArray *)newBulletsForLocation:(CGPoint)location direction:(NSInteger)direction {
    ...
    CGPoint t1 = CGPointMake(location.x-(direction * self.speed), location.y+(direction * 3.3 *self.speed));
    b.vel = GetAngle(location, t1);
    b.vel = MultipliedPoint(b.vel, self.speed);
    
    CGPoint t2 = CGPointMake(location.x+(direction * self.speed), location.y+(direction * 3.3 *self.speed));
